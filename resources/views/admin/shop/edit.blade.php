...
<div class="container-fluid px-4 py-6">
    ...
    <div class="mb-6">
        <div class="flex items-center gap-3 mb-2">
            ...
            <h1 class="text-3xl font-bold text-gray-800">تعديل منتج: {{ $item->name }}</h1>
        </div>
        <p class="text-gray-600">تعديل معلومات المنتج</p>
    </div>

    <form action="{{ route('admin.shop.update', $item->id) }}" method="POST" enctype="multipart/form-data" class="max-w-4xl">
        @csrf
        @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" rows="3" 
                                  class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                  placeholder="وصف مختصر للمنتج...">{{ old('description', $item->description) }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="type" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                            ...
                            <option value="power_up" {{ old('type', $item->type) == 'power_up' ? 'selected' : '' }}>تعزيز</option>
                            <option value="special" {{ old('type', $item->type) == 'special' ? 'selected' : '' }}>خاص</option>
                        </select>
                        @error('type')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="rarity" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                            ...
                            <option value="legendary" {{ old('rarity', $item->rarity) == 'legendary' ? 'selected' : '' }}>🟡 خرافي</option>
                        </select>
                        @error('rarity')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="number" name="price" value="{{ old('price', $item->price) }}" min="1"
                                   class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    ...
                    <div>
                        ...
                        <input type="text" name="icon" value="{{ old('icon', $item->icon) }}" maxlength="10"
            ...
            <div class="p-6 border-b border-gray-200">
                ...
                <div>
                    ...
                    <div id="imagePreview" class="mt-4 hidden">
                        <p class="text-sm font-semibold text-gray-700 mb-2">معاينة الصورة الجديدة:</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="is_limited" value="1" {{ old('is_limited', $item->is_limited) ? 'checked' : '' }}
                    ...
                    <!-- Available Until -->
                    <div id="limitedDateContainer" class="md:col-span-2 {{ old('is_limited', $item->is_limited) ? '' : 'hidden' }}">
                        <label class="block text-sm font-semibold text-gray-700 mb-2">
                            متاح حتى تاريخ
                        </label>
                        <input type="datetime-local" name="available_until" 
                               value="{{ old('available_until', $item->available_until ? $item->available_until->format('Y-m-d\TH:i') : '') }}"
                               class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            ...
            <div class="p-6">
                ...
                <div>
                    ...
                    <select name="status" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                        ...
                        <option value="inactive" {{ old('status', $item->status) == 'inactive' ? 'selected' : '' }}>غير نشط</option>
                        <option value="sold_out" {{ old('status', $item->status) == 'sold_out' ? 'selected' : '' }}>نفذ من المخزون</option>
                    </select>
                    @error('status')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
