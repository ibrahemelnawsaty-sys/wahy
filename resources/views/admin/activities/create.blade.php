        ...
        <div class="form-grid">
            ...
            <div class="project-fields" style="display: none; grid-column: 1 / -1;">
                <div style="background: #fef3c7; border: 2px solid #f59e0b; border-radius: 12px; padding: 20px; margin-bottom: 20px;">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="max_file_size_mb" class="form-input" value="{{ old('max_file_size_mb', 10) }}" min="1" max="100" placeholder="10">
                    </div>
                </div>
            </div>
            ...
            <div class="form-group full-width">
                <label style="display:flex;align-items:center;gap:12px;padding:16px;background:#eef2ff;border:2px solid #6366f1;border-radius:10px;cursor:pointer;">
                    <input type="checkbox" name="requires_parent_approval" value="1" {{ old('requires_parent_approval') ? 'checked' : '' }} style="width:20px;height:20px;cursor:pointer;accent-color:#6366f1;">
                    <span>
...
function uploadActivityImage(input, qIndex, imgIndex) {
    ...
    .then(function(data) {
        // نضمن رابطاً مطلقاً: حقل type=url يرفض النسبيّ، ونحتاجه http(s) لواجهة الطالب/المراجعة
