...
<div class="parents-header">
    <div>
        <h2 style="margin: 0 0 8px 0;">👪 إدارة أولياء الأمور</h2>
        <p style="margin: 0; color: #64748b;">عرض وإدارة جميع أولياء الأمور</p>
    </div>
    <a href="{{ route('admin.parents.create') }}" class="btn-add">➕ إضافة ولي أمر</a>
</div>
...
<div class="parents-table">
            ...
            <tr>
                <td>
                    <div class="parent-info">
                        <div class="parent-avatar">{{ mb_substr($parent->name, 0, 1, "UTF-8") }}</div>
                        <div>
                            <div style="font-weight: 600;">{{ $parent->name }}</div>
                            <div style="font-size: 13px; color: #64748b;">{{ $parent->email }}</div>
                        </div>
                    </div>
                </td>
                <td>{{ $parent->school->name ?? '-' }}</td>
                <td>{{ $parent->phone }}</td>
                <td><code style="background: #f1f5f9; padding: 4px 8px; border-radius: 4px;">{{ $parent->qr_code }}</code></td>
                <td><span class="status-{{ $parent->status }}">{{ $parent->status == 'active' ? 'نشط' : 'غير نشط' }}</span></td>
                <td>
                    <div style="display: flex; gap: 8px;">
                        ...
                        <form method="POST" action="{{ route('admin.parents.destroy', $parent) }}" style="display: inline;" onsubmit="return confirm('هل أنت متأكد؟')">
                            ...
                            <button type="submit" class="btn-action btn-delete"
                                    aria-label="حذف ولي الأمر {{ $parent->name }}" title="حذف">🗑️</button>
                        </form>
                    </div>
                </td>
            </tr>
    ...
    <div style="padding: 20px;">{{ $parents->links() }}</div>
    @else
