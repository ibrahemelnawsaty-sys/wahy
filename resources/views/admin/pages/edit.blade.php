...
@section('content')
    <form action="{{ route('admin.pages.update', $page->id) }}" method="POST" id="pageBuilderForm">
        @csrf
        @method('PUT')
            ...
            <div class="admin-card-body" style="padding: 24px;">
                <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 24px;">
                    <div class="admin-form-group">
                        ...
                        <input type="text" name="page_name" class="admin-form-input" required value="{{ $page->page_name }}">
                    </div>

                    <div class="admin-form-group">
                        <label class="admin-form-label">الرابط (Slug) *</label>
                        <input type="text" name="slug" class="admin-form-input" required value="{{ $page->slug }}">
                        <div class="admin-form-help">سيكون الرابط: example.com/<strong>{{ $page->slug }}</strong></div>
                    </div>

                    <div class="admin-form-group">
                        <label class="admin-form-label">عنوان SEO</label>
                        <input type="text" name="meta_title" class="admin-form-input" value="{{ $page->meta_title }}">
                    </div>
                    ...
                    <div class="admin-form-group" style="grid-column: 1 / -1;">
                        ...
                        <textarea name="meta_description" class="admin-form-textarea" rows="3">{{ $page->meta_description }}</textarea>
                    </div>
                </div>
            </div>
        </div>
...
let pageBlocks = @json($page->json_data);
let draggedElement = null;
let blockIdCounter = pageBlocks.length > 0 ? Math.max(...pageBlocks.map(b => b.id)) : 0;

// رسم البلوكات الموجودة عند التحميل
...
function renderBlockContent(block) {
    switch(block.type) {
        ...
        case 'spacer':
            return `<div style="height: ${block.content.height}; background: repeating-linear-gradient(45deg, #f8fafc, #f8fafc 10px, #e2e8f0 10px, #e2e8f0 20px);"></div>`;
        default:
