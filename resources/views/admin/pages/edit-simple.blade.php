...
@section('page-title', 'تعديل صفحة: ' . $page->page_name)
    ...
    <form action="{{ route('admin.pages.update', $page->id) }}" method="POST">
        ...
        <div class="admin-form-grid">
            <div class="admin-form-group">
                ...
                <input type="text" id="page_name" name="page_name" class="admin-input" value="{{ $page->page_name }}" required>
            </div>

            <div class="admin-form-group">
                <label for="slug">الرابط (Slug)</label>
                <input type="text" id="slug" name="slug" class="admin-input" value="{{ $page->slug }}" required>
            </div>
        </div>

        <div class="admin-form-group">
            <label for="meta_title">عنوان الصفحة (Meta Title)</label>
            <input type="text" id="meta_title" name="meta_title" class="admin-input" value="{{ $page->meta_title }}">
        </div>

        <div class="admin-form-group">
            <label for="meta_description">وصف الصفحة (Meta Description)</label>
            <textarea id="meta_description" name="meta_description" class="admin-input" rows="3">{{ $page->meta_description }}</textarea>
        </div>
        ...
        <h3>محتوى الصفحة</h3>
        <p style="color: #64748b; margin-bottom: 24px;">عدّل محتوى الصفحة باستخدام JSON</p>

        <div class="admin-form-group">
            <label for="json_data">JSON Data</label>
            <textarea id="json_data" name="json_data" class="admin-input" rows="20" required style="font-family: monospace; direction: ltr;">{{ json_encode($page->json_data, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</textarea>
