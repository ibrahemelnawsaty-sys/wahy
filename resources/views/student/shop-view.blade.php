    ...
    <div class="rewards-grid">
        @forelse($shopItems as $index => $item)
            ...
            <div class="reward-card scale-in {{ !$canAfford || $hasPurchased ? 'disabled' : '' }}"
                ...
                @if($item->image)
                    <img src="{{ asset('storage/data/' . $item->image) }}" alt="{{ $item->name }}" style="width: 100%; height: 150px; object-fit: cover; border-radius: 12px; margin-bottom: 16px;">
                @else
                    <div class="reward-icon-large {{ $__sfa ? 'wahy-frame wahy-frame-' . $__sfa : '' }}" style="{{ $__sfa ? 'width:96px;height:96px;border-radius:50%;margin:0 auto 16px;display:flex;align-items:center;justify-content:center;' : '' }}">{{ $item->icon }}@if($__sfa)@include('partials.wf-particles')@endif</div>
                @endif
