...
class PointsDistributionService
{
    ...
    /**
     * توزيع لينِن (legacy): كل طرف معزول بـ try/catch — فشل طرف لا يُفشل البقية ولا
     * يُرجِع استثناءً. يُستخدم في المسارات بعد commit حيث لا توجد معاملة محيطة لتُلَفّ.
     */
    public function distribute(User $student, int $points, string $source, string $description): void
    {
        ...
        foreach (['awardTeacher', 'awardParent', 'awardSchool'] as $method) {
            try {
                $this->{$method}($student, $points, $source, $description);
            } catch (\Throwable $e) {
                Log::warning("Points distribution {$method} failed", [
    ...
    /**
     * توزيع صارم (atomic): يُستخدم من داخل معاملة AwardService. لا يبتلع الأخطاء —
     * أي فشل في طرف يُرجِع استثناءً فتتراجع المعاملة كاملةً (بما فيها منح الطالب) —
     * لا توزيع جزئي.
     */
    ...
    private function awardSchool(User $student, int $points, string $source, string $description): void
    {
        ...
        // schools.total_points هو عدّاد حيّ حقيقي (بخلاف users.total_points الميت).
