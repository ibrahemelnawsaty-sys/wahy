...
class AuthController extends Controller
{
    ...
    public function showTwoFactorVerify(Request $request)
    {
        ...
        // **يُمنع** regenerateToken() هنا. الجلسة تحتفظ بـ`_token` واحد، وتدويره في معالج **قراءة**
        // يجعل الصفحة أحاديّة الاستعمال: أيّ فتحٍ ثانٍ للرابط (إعادة تحميل أثناء انتظار البريد
        // المُصفَّف، أو إعادة تسجيل دخول، أو تبويب ثانٍ، أو جلبٌ استباقيّ من المتصفّح، أو إعادة
        // تحميل التبويب بعد العودة من تطبيق البريد) يقتل الرمز في النسخة المعروضة ⟶ 419.
        // والتبرير القديم («تجديد الجلسة لئلّا تنتهي») غير صحيح: Store::regenerateToken يكتب
        // `_token` فقط ولا يمسّ عمر الجلسة ولا الكوكي. ولا يُخسر أمن: تدوير الجلسة يقع في موضعه
