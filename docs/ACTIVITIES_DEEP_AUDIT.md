...
## ✅ حالة المعالجة (2026-08-25) — أُصلِح 18 من 20؛ الحزمة 738/738 خضراء
...
**مؤجَّلان بقرارٍ واعٍ (منخفضان، ليسا عطبَي صحّة/ازدواج):**
- **L4** — كاتبان على `teacher_points.points` بصيغتَي مصدر/تقريب مختلفتين (تذبذبٌ بصريّ لا مضاعفة). التوحيد الآمن = جعل `updateTeacherPoints` (الحاوي لمكافآت الأنشطة/الأسئلة) المصدرَ الوحيد، لكنّه **يغيّر قيمة نقاط المعلّم** (يضيف المكافآت) فيحتاج قرار منتج + تحديث اختبارات — لا يُفرَض ضمن إصلاح أخطاء.
- **L9** — الحدّ الزمنيّ للاختبار الموقوت غير مفروض في الجوّال (آليّة `session` الخادميّة غير متاحة لـsanctum عديم الحالة). الفرض يتطلّب ختم بدءٍ مخزَّناً عند جلب النشاط — تحسينٌ معماريّ مستقلّ. الويب يفرضه.
