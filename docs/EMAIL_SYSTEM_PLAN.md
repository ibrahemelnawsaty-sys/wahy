> ## ⚠️ تصحيح جوهريّ (2026-08-19) — اقرأه قبل أيّ اعتماد على ما تحته
>
> **ما كان مكتوباً هنا خطأ، ومصدره استنتاجٌ لا ملاحظة.** الوثيقة تُثبِت «لا عامل طابور يعمل على
> الإنتاج» وتنسب ذلك إلى تأكيد commit `9d8c9a8`. لكنّ ذلك الكوميت لم يرصد سلوكاً على الإنتاج —
> بل استنتجه من تباينٍ في الكود (الدخول `queue()` وإعادة الإرسال `send()`)، ثمّ صار الاستنتاج
> يُقتبَس بوصفه دليلاً.
>
> **الدليل الحقيقيّ يقول العكس:**
> 1. المالك رصد أنّ **حملات البريد تصل**.
> 2. `CampaignMail implements ShouldQueue` (app/Mail/CampaignMail.php:17).
> 3. `Mailer::sendMailable()` (vendor/laravel/framework/src/Illuminate/Mail/Mailer.php:349-353)
>    يُحوّل **أيّ** Mailable من نوع ShouldQueue إلى `queue()` حتى لو نوديت `send()`.
>
> ⟹ لا يوجد مسار حملة متزامن إطلاقاً. ووصولها يُثبت أنّ **عامل الطابور يعمل ويُفرِّغ `jobs`**.
>
> **والعطل الحقيقيّ عكس ما وُثِّق:** عمليّة الويب (PHP-FPM) هي التي تعجز عن فتح الاتّصال:
> `Connection could not be established with host "smtp.office365.com:587" ... (Connection timed out)`
> — ظهر في سجلّ البريد لبريد نموذج التواصل المتزامن. وليس حجباً شبكيّاً للمنفذ (العامل على
> نفس الخادم ويصل SMTP بنجاح)، بل **اختلاف إعدادات PHP بين SAPI الويب وCLI**
> (`disable_functions` / `open_basedir` / `allow_url_fopen`).
>
> **الخلاصة المُعتمَدة:** الطابور هو المسار الوحيد العامل ⟹ كلّ بريد يُصفَّف (نُفِّذ في `72209b4`).
> وتحويل 2FA إلى `send()` في `fb2d7bb` كان **خطأً** أُعيد عنه.
